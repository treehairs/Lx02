    ...
    public class UserInfo
    {
        ...
        public Model.UserInfo GetModel(string UserName)
        {
            using(IDbConnection cn= new MySqlConnection(cns))
            {
                ...
                return cns.QueryFirstOrDefault<Model.UserInfo>(sql, new { username = UserName });
            }
        }
        ...
        public IEnumerable<Model.UserInfoNo>GetPage(Model.Page page)
        {
            using (IDbConnection cn = new MySqlConnection(cns))
            {
                ...
                return cn.Query<Model.UserInfoNo>(sql);
            }
        }
        public int Add(Model.UserInfo user)
        {
            using (IDbConnection cn = new MySqlConnection(cns))
            {
                string sql = "insert into userinfo valies(@userName,@passWord,@qq,@email,@email,@type,@userImg);";
        ...
        public int Delete(string username)
        {
            using (IDbConnection cn = new MySqlConnection(cns))
            {
                string sql = "delete form userinfo where username=@username";
                return cn.Execute(sql, new MySqlConnection(cns));
            }
        }
    }
}
